...
// //         setStatus(value);
// //         const res = await fetch("http://localhost:5000/chstatus",{
// //             method:"POST",
...
// //                 {/* Quick Actions */}
// //                 {/* <Button className="quick-actions-btn" variant="primary" onClick={handleShow}>
// //                     ⚡ Quick Actions
// //                 </Button> */}
...
//   useEffect(() => {
//     if (!us?.userEmail) return

//     const sameUser = users.filter(u => u.userEmail === us.userEmail)

//     const statusCount = _.countBy(sameUser, "status")
//     setResolved(statusCount.Resolved || 0)
//     setProgress(statusCount["In Progress"] || 0)
//     setPending(statusCount.Pending || 0)

//     const complaintCount = _.countBy(sameUser, "complaint")
//     const formatted = Object.keys(complaintCount).map(key => ({
//       Name: key,
//       Status: complaintCount[key]
//     }))
...
//   const handleSelect = async (value) => {
//     setStatus(value)
//     await fetch("http://localhost:5000/chstatus", {
//       method: "POST",
//       headers: { "Content-Type": "application/json" },
//       body: JSON.stringify({
//         email: us.userEmail,
...
//           <Link href="/adcom">📋 All Complaints</Link>
//           <Link href="/report">🔔 Report</Link>
//           <Link href="/notia">💬 Notification</Link>
//           <Link href="/adp">👤 Profile</Link>
//         </nav>
//       </div>
...
//         <p>{us?.userEmail}</p>
//         <p>📍 {us?.location}</p>
//         <p>🕒 {us?.uploadedAt && new Date(us.uploadedAt).toLocaleString()}</p>

//         {/* STATS */}
//         <div className="stats-overview">
//           <div>✅ Resolved: {resolved}</div>
//           <div>🔄 In Progress: {progress}</div>
...
//             <YAxis />
//             <Tooltip />
//             <Area dataKey="Status" stroke="#667eea" fill="#667eea" />
//           </AreaChart>
//         </ResponsiveContainer>
...
function InfoContent() {
  ...
  useEffect(() => {
    ...
    const complaintCount = _.countBy(sameUser, "complaint")
    setChartData(
      Object.keys(complaintCount).map(k => ({
        Name: k,
        Status: complaintCount[k]
      }))
    )
  }, [us, users])
  ...
  return (
    <div className="super-complaint-detail">
      <div className="premium-sidebar">
        ...
        <Link href="/adcom">📋 Complaints</Link><br/>
        <Link href="/report">🔔 Report</Link><br/>
        <Link href="/notia">💬 Notification</Link><br/>
        <Link href="/adp">👤 Profile</Link>
      </div>
